...
class Normal(nn.Module):
    ...
    def sample_contrlVAE(self, size=None, params=None):
        mu, logsigma = self._check_inputs(size, params)
        std_z = Variable(torch.randn(mu.size()).type_as(mu.data))
        sample = std_z * torch.exp(logsigma) + mu
        return sample, mu, logsigma 

    def log_density(self, sample, params=None,display = False):
        ...
        if display:
            print("mu",mu,"sample",sample,"(sample - mu)",(sample - mu))
...
class Bernoulli(nn.Module):
    ...
    def log_density(self, sample, params=None):#从sample重建x的损失
