...
DSPRITES_PATH = "E:/python3.8.10/disentanglement_lib-master/bin/dsprites/dsprites_ndarray_co1sh3sc6or40x32y32_64x64.npz"
...
class DSprites(ground_truth_data.GroundTruthData):
  ...
  def sample_observations_from_factors_no_color(self, factors, random_state):
    ...
    return torch.from_numpy(self.images[indices]).unsqueeze(1).float()
    #return torch.from_numpy(np.expand_dims(self.images[indices].astype(np.float32), axis=3)).float().transpose(1, 3).transpose(2, 3)  
