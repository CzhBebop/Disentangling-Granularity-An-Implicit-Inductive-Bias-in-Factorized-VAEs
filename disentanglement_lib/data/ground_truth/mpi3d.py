...
import torch
mpi3d_path = "E:\\python3.8.10\\disentanglement_lib-master\\bin\\mpi3d_real\\mpi3d_real.npz"
...
class MPI3D(ground_truth_data.GroundTruthData):
  ...
  def __init__(self, mode="mpi3d_realistic"):
    if mode == "mpi3d_toy":
    ...
    self.images = (
        self.images.reshape([460800, 64, 64, 3]).astype(np.float32) / 255.)
  ...
  def sample_observations_from_factors(self, factors, random_state):
    ...
    return torch.from_numpy(self.images[indices]).float().transpose(1, 3).transpose(2, 3)
