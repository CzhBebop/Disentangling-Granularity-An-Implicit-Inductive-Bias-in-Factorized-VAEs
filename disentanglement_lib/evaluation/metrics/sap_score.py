...
def compute_sap(ground_truth_data,
  ...
  return _compute_sap(mus, ys, mus_test, ys_test, continuous_factors,split_num)


def _compute_sap(mus, ys, mus_test, ys_test, continuous_factors,split_num):
  """Computes score based on both training and testing codes and factors."""
  score_matrix = compute_score_matrix(mus, ys, mus_test,
                                      ys_test, continuous_factors,split_num)
  # Score matrix should have shape [num_latents, num_factors].
  assert score_matrix.shape[0] ==int(mus.shape[0]/split_num)
  assert score_matrix.shape[1] == ys.shape[0]
  scores_dict = {}
  dele_col = []
  
  for col in range(ys.shape[0]):
    #print("col",col,score_matrix[:,col])
    if not(np.any(score_matrix[:,col])):
      #score_matrix = np.delete(score_matrix, col, axis=1)
      dele_col.append(col)
  score_matrix = np.delete(score_matrix, dele_col, axis=1)
...
def compute_sap_on_fixed_data(observations, labels, representation_function,split_num=1,
  ...
  return _compute_sap(mus_train, ys_train, mus_test, ys_test,
                      continuous_factors,split_num)


def compute_score_matrix(mus, ys, mus_test, ys_test, continuous_factors,split_num):
  """Compute score matrix as described in Section 3."""
  num_latents = int(mus.shape[0]/split_num)
  ...
  for i in range(num_latents):
    for j in range(num_factors):
      ...
      if continuous_factors:
        #nogood Attribute is considered continuous.
        cov_mu_i_y_j = np.cov(mu_i, y_j, ddof=1)
        cov_mu_y = cov_mu_i_y_j[0, 1]**2
        var_mu = cov_mu_i_y_j[0, 0]
        var_y = cov_mu_i_y_j[1, 1]  
        if var_mu > 1e-8 and var_y > 1e-8:
        ...
        if not(np.any(y_j)):
          score_matrix[i, j] = 0
          print("all zero factor: No i-j ",i,j)
          continue
        ...
        if split_num ==1:
          ...
          classifier.fit(mu_i.transpose(), y_j)
          pred = classifier.predict(mu_i_test.transpose())
