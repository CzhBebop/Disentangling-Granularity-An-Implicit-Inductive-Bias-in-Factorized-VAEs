...
class QuantizedLatent(nn.Module):
    def __init__(self, num_latents, num_values_per_latent, optimize_values , key = None):
        ...
        def pquant(x, svpl_T):
            """
            x:        [B, L]  批次×latent维数
            svpl_T:   [K, L]  每个latent对应的K个候选值（已转置）
            返回:
            quantized: [B, L]
            indices:   [B, L]  每个样本、每个latent对应的索引
            """
            # 确保是2D
            if x.dim() == 1:
            ...
            B, L = x.shape
            K, L2 = svpl_T.shape
            assert L == L2, f"Dim mismatch: x has L={L}, svpl_T has L={L2}"

            q_chunks = []
            idx_chunks = []
            for i in range(L):
                # 取出第 i 个 latent 的输入 [B] 和对应的 codebook 候选值 [K]
                xi = x[:, i]
                code_i = svpl_T[:, i]
                # 调用你原本的标量/向量版本 quantize
                qi, idi = self.quantize(xi, code_i)  # 期望返回 [B], [B]
                # 保持维度对齐
                q_chunks.append(qi.unsqueeze(1))
                idx_chunks.append(idi.unsqueeze(1))

            quantized = torch.cat(q_chunks, dim=1)   # [B, L]
            indices   = torch.cat(idx_chunks, dim=1) # [B, L]
