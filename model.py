...
class unGuidedVAE(nn.Module):
    def __init__(self,raw_width = 28,raw_channel = 1,raw_liner_dim = 784,n_vae_dis=10, DIM=64):
        super().__init__()
        self.q_dist=dist.Normal()
        self.DIM = DIM
        self.z_dim = n_vae_dis
        self.raw_liner_dim = raw_liner_dim
        self.channel = raw_channel
        self.layer0 = nn.Conv2d(raw_channel, 10, 10, stride=2)
        self.layer1 = nn.Conv2d(10, DIM, 6, stride=2)
        ...
        self.deconv_out = nn.ConvTranspose2d(DIM, 10, 6, stride=2)
        self.deconv_out0 = nn.ConvTranspose2d(10, raw_channel, 10, stride=2)

        self.fc1 = nn.Linear(n_vae_dis - 2, self.raw_liner_dim)
    ...
    def encode(self, x):
        ...
        x = x.contiguous().view(-1, self.DIM*4*4*4)
    ...
    def decode(self, z):
        ...
        output = self.deconv_out0(output)
        if self.channel==1:
            output = torch.sigmoid(output)
        return output.view(-1, self.raw_liner_dim)
    ...
    def light_decode(self, z):
        ...
        device = z.device
        if self.channel==1:
            img_raw = torch.sigmoid(self.fc1(z_content).view(-1, self.channel, self.width, self.width))
        else:
            img_raw =self.fc1(z_content).view(-1, self.channel, self.width, self.width)
...
class suGuidedVAE(nn.Module):
    def __init__(self, n_chanl=3 ,n_vae_dis=16):
        super().__init__()

        self.n_vae_dis = n_vae_dis
        self.n_vae_channel = n_chanl  
        self.encoder = nn.Sequential(
            nn.Conv2d(self.n_vae_channel, 32, 4, 2, 1),
            nn.ReLU(True),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.ConvTranspose2d(32, self.n_vae_channel, 4, 2, 1),
        )
