...
def computeMIG(COUNT = 0,DATANAME='car'):
    ...
    if MO == "VQ":
        ...
        model.load_state_dict(torch.load('disentanglement_metric\\vqvae_model\\best_model_'+DATANAME+'_'+str(COUNT)+'.pth'))
    elif MO == "QL":
        from qlae import Model
        model = Model().to(device)
        model.load_state_dict(torch.load('disentanglement_metric\\results\\shapes3d\\trial'+str(COUNT)+'\\model'+str(COUNT)+'.pt'))
    ...
    if MO == "VQ":
        ...
        print(" code perplexity:", avg_perp, "(MAX:", model.vq.num_embeddings, ")")
        ...
        print("AVG code perplexity:", avg_perp, "(MAX:", 12, ")")
        #model.vq.num_embeddings
        # 用“离散下标”计算 MIG（不用分箱了）
        mig_idx, gaps_idx, MI_idx = compute_mig_from_indices(enc_idx_all, factor_cards_demo)
        print("MIG (from indices):", mig_idx)
